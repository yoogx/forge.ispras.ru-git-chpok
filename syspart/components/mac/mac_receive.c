...
ret_t mac_receive(MAC_RECEIVER *self, const uint8_t *data, size_t len)
{
    ...
    if (ether_hdr->ethertype == hton16(ETH_P_ARP)) {
        return MAC_RECEIVER_call_port_ARP_handle(self, (const uint8_t *)ether_hdr->payload, len);
    } else if(ether_hdr->ethertype == hton16(ETH_P_IP)){
        return MAC_RECEIVER_call_port_UDP_handle(self, (const uint8_t *)ether_hdr->payload, len);
    } else {
