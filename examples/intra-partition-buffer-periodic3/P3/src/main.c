...
static void first_process(void)
{
    ...
    while (1) {
        ...
        if (ret != NO_ERROR) {
            printf("P3: couldn't send to the buffer: %d\n", (int) ret);
            break;
        } 
        i++;
        printf("P3: PERIODIC_WAIT[%d]\n", i);
        PERIODIC_WAIT(&ret);
        printf("P3: AFTER PERIODIC_WAIT[%d]\n", i);
    }
    STOP_SELF();
}
...
static void second_process(void)
{
    ...
    while (1) {
        ...
        if (ret != NO_ERROR) {
            printf("P3: couldn'd receive from the buffer: %d\n", (int) ret);
            break;
        }
        printf("P3: received message %d\n", i);
    }
    STOP_SELF();
}
...
static int real_main(void)
{
    ...
    if (ret != NO_ERROR) {
        printf("P3: error creating a buffer: %d\n", (int) ret);
        return 1;
    } else {
        printf("P3: buffer successfully created\n");
    }
    global_buffer_id = id;
    ...
    if (ret != NO_ERROR) {
        printf("P3: couldn't create process 1: %d\n", (int) ret);
        return 1;
    } else {
        printf("P3: process 1 created\n");
    }
    
    START(pid, &ret);
    if (ret != NO_ERROR) {
        printf("P3: couldn't start process 1: %d\n", (int) ret);
        return 1;
    } else {
        printf("P3: process 1 \"started\" (it won't actually run until operating mode becomes NORMAL)\n");
    }
    ...
    if (ret != NO_ERROR) {
        printf("P3: couldn't create process 2: %d\n", (int) ret);
        return 1;
    } else {
        printf("P3: process 2 created\n");
    }

    START(pid, &ret);
    if (ret != NO_ERROR) {
        printf("P3: couldn't start process 2: %d\n", (int) ret);
        return 1;
    } else {
        printf("P3: process 2 \"started\" (it won't actually run until operating mode becomes NORMAL)\n");
    }
    ...
    if (ret != NO_ERROR) {
        printf("P3: couldn't transit to normal operating mode: %d\n", (int) ret);
    } 
