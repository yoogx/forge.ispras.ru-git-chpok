...
#define RS_232C_REG_THR 0
#define RS_232C_REG_LSR 5

#define UART_LSR_THRE   0x20


static void RS_232C_writeb(int offset, int value, int flag)
{
    if (flag == 0){
...
static int RS_232C_readb(int offset, int flag)
{
    if (flag == 0) {
...
{
   while ((RS_232C_readb(RS_232C_REG_LSR, 1) & UART_LSR_THRE) == 0)
     ;
   RS_232C_writeb(RS_232C_REG_THR, a, 1);
}

static void write_serial_0(char a)
{
   while ((RS_232C_readb(RS_232C_REG_LSR, 0) & UART_LSR_THRE) == 0)
     ;


   RS_232C_writeb(RS_232C_REG_THR, a, 0);
}
...
{
    int flags = RS_232C_readb(RS_232C_REG_LSR, 0);
...
{
    int flags = RS_232C_readb(RS_232C_REG_LSR, 1);
    if ((!(flags & UART_LSR_DR)) || (flags & UART_LSR_RFE))
        return 0;
    return 1;
}
...
{
       int data;
       data=RS_232C_readb(RS_232C_REG_THR,0);
       if ( !(RS_232C_readb(RS_232C_REG_LSR,0) & UART_LSR_RFE) )
               return data;
       return -1;
}
...
{
    int data;
    data=RS_232C_readb(RS_232C_REG_THR,1);
    if ( !(RS_232C_readb(RS_232C_REG_LSR,1) & UART_LSR_RFE) )
        return data;
    return -1;
}
