...
jet_ret_t _pok_thread_create_impl(const char* name,
    void* entry,
...
jet_ret_t pok_thread_create(const char* name,
    void* entry,
    const pok_thread_attr_t* attr,
    pok_thread_id_t* thread_id)
__attribute__ ((weak, alias ("_pok_thread_create_impl")));

jet_ret_t _pok_thread_sleep_impl(const pok_time_t* time)
{
    return pok_syscall1(POK_SYSCALL_THREAD_SLEEP,
        (uint32_t)time);
}

jet_ret_t pok_thread_sleep(const pok_time_t* time)
__attribute__ ((weak, alias ("_pok_thread_sleep_impl")));

jet_ret_t _pok_sched_end_period_impl(void)
{
...
jet_ret_t _pok_thread_suspend_impl(const pok_time_t* time)
{
...
jet_ret_t _pok_thread_get_status_impl(pok_thread_id_t thread_id,
    char* name,
    void** entry,
...
jet_ret_t _pok_thread_delayed_start_impl(pok_thread_id_t thread_id,
...
jet_ret_t _pok_thread_set_priority_impl(pok_thread_id_t thread_id,
    uint32_t priority)
{
...
jet_ret_t _pok_thread_resume_impl(pok_thread_id_t thread_id)
{
...
jet_ret_t _pok_thread_suspend_target_impl(pok_thread_id_t thread_id)
{
...
jet_ret_t _pok_thread_yield_impl(void)
{
...
jet_ret_t _pok_sched_replenish_impl(const pok_time_t* budget)
{
    return pok_syscall1(POK_SYSCALL_THREAD_REPLENISH,
        (uint32_t)budget);
}

jet_ret_t pok_sched_replenish(const pok_time_t* budget)
__attribute__ ((weak, alias ("_pok_sched_replenish_impl")));

jet_ret_t _pok_thread_stop_target_impl(pok_thread_id_t thread_id)
{
...
jet_ret_t _pok_thread_stop_impl(void)
{
...
jet_ret_t _pok_thread_find_impl(const char* name,
    pok_thread_id_t* id)
{
...
jet_ret_t pok_thread_find(const char* name,
    pok_thread_id_t* id)
__attribute__ ((weak, alias ("_pok_thread_find_impl")));


jet_ret_t _jet_resched_impl(void)
{
...
jet_ret_t _jet_msection_enter_helper_impl(struct msection* section)
{
    return pok_syscall1(POK_SYSCALL_MSECTION_ENTER_HELPER,
        (uint32_t)section);
}

jet_ret_t jet_msection_enter_helper(struct msection* section)
__attribute__ ((weak, alias ("_jet_msection_enter_helper_impl")));

jet_ret_t _jet_msection_wait_impl(struct msection* section,
...
jet_ret_t jet_msection_wait(struct msection* section,
    const pok_time_t* timeout)
__attribute__ ((weak, alias ("_jet_msection_wait_impl")));

jet_ret_t _jet_msection_notify_impl(struct msection* section,
    pok_thread_id_t thread_id)
{
...
jet_ret_t jet_msection_notify(struct msection* section,
    pok_thread_id_t thread_id)
__attribute__ ((weak, alias ("_jet_msection_notify_impl")));

jet_ret_t _jet_msection_wq_notify_impl(struct msection* section,
...
jet_ret_t jet_msection_wq_notify(struct msection* section,
    struct msection_wq* wq,
    pok_bool_t is_all)
__attribute__ ((weak, alias ("_jet_msection_wq_notify_impl")));

jet_ret_t _jet_msection_wq_size_impl(struct msection* section,
    struct msection_wq* wq,
    size_t* size)
{
...
jet_ret_t jet_msection_wq_size(struct msection* section,
    struct msection_wq* wq,
    size_t* size)
__attribute__ ((weak, alias ("_jet_msection_wq_size_impl")));


jet_ret_t _pok_partition_set_mode_current_impl(pok_partition_mode_t mode)
{
...
jet_ret_t _pok_current_partition_get_status_impl(pok_partition_status_t* status)
{
...
jet_ret_t _pok_current_partition_inc_lock_level_impl(int32_t* lock_level)
{
...
jet_ret_t _pok_current_partition_dec_lock_level_impl(int32_t* lock_level)
{
...
jet_ret_t _pok_error_thread_create_impl(uint32_t stack_size,
    void* entry)
{
...
jet_ret_t _pok_error_raise_application_error_impl(const char* msg,
    size_t msg_size)
{
...
jet_ret_t pok_error_raise_application_error(const char* msg,
    size_t msg_size)
__attribute__ ((weak, alias ("_pok_error_raise_application_error_impl")));

jet_ret_t _pok_error_get_impl(pok_error_status_t* status,
    void* msg)
{
...
jet_ret_t pok_error_get(pok_error_status_t* status,
    void* msg)
__attribute__ ((weak, alias ("_pok_error_get_impl")));

jet_ret_t _pok_error_raise_os_error_impl(const char* msg,
    size_t msg_size)
{
...
jet_ret_t pok_error_raise_os_error(const char* msg,
    size_t msg_size)
__attribute__ ((weak, alias ("_pok_error_raise_os_error_impl")));


   /* Middleware syscalls */
jet_ret_t _pok_port_sampling_create_impl(const char* name,
...
jet_ret_t pok_port_sampling_create(const char* name,
    pok_port_size_t size,
    pok_port_direction_t direction,
    const pok_time_t* refresh,
    pok_port_id_t* id)
...
jet_ret_t _pok_port_sampling_write_impl(pok_port_id_t id,
    const void* data,
    pok_port_size_t len)
{
...
jet_ret_t _pok_port_sampling_read_impl(pok_port_id_t id,
    void* data,
    pok_port_size_t* len,
    pok_bool_t* valid)
{
...
jet_ret_t _pok_port_sampling_id_impl(const char* name,
    pok_port_id_t* id)
{
...
jet_ret_t pok_port_sampling_id(const char* name,
    pok_port_id_t* id)
__attribute__ ((weak, alias ("_pok_port_sampling_id_impl")));

jet_ret_t _pok_port_sampling_status_impl(pok_port_id_t id,
...
jet_ret_t _pok_port_sampling_check_impl(pok_port_id_t id)
{
...
jet_ret_t _pok_port_queuing_create_packed_impl(const char* name,
...
jet_ret_t pok_port_queuing_create_packed(const char* name,
    const pok_port_queuing_create_arg_t* arg,
    pok_port_id_t* id)
__attribute__ ((weak, alias ("_pok_port_queuing_create_packed_impl")));

jet_ret_t _pok_port_queuing_send_impl(pok_port_id_t id,
    const void* data,
    pok_port_size_t len,
    const pok_time_t* timeout)
{
...
jet_ret_t _pok_port_queuing_receive_impl(pok_port_id_t id,
    const pok_time_t* timeout,
    void* data,
    pok_port_size_t* len)
{
...
jet_ret_t _pok_port_queuing_id_impl(const char* name,
    pok_port_id_t* id)
{
...
jet_ret_t pok_port_queuing_id(const char* name,
    pok_port_id_t* id)
__attribute__ ((weak, alias ("_pok_port_queuing_id_impl")));

jet_ret_t _pok_port_queuing_status_impl(pok_port_id_t id,
...
jet_ret_t _pok_port_queuing_clear_impl(pok_port_id_t id)
{
...
jet_ret_t _jet_memory_block_get_status_impl(const char* name,
...
jet_ret_t jet_memory_block_get_status(const char* name,
    jet_memory_block_status_t* status)
__attribute__ ((weak, alias ("_jet_memory_block_get_status_impl")));

jet_ret_t _jet_ippc_partition_arinc_init_portal_impl(const char* portal_name,
    int* portal_id)
{
...
jet_ret_t jet_ippc_partition_arinc_init_portal(const char* portal_name,
    int* portal_id)
__attribute__ ((weak, alias ("_jet_ippc_partition_arinc_init_portal_impl")));

jet_ret_t _jet_ippc_partition_arinc_call_impl(int portal_id,
...
jet_ret_t _jet_ippc_partition_arinc_get_portal_type_info_impl(const char* portal_name,
    int* portal_type_id,
    int* n_clients)
{
...
jet_ret_t jet_ippc_partition_arinc_get_portal_type_info(const char* portal_name,
    int* portal_type_id,
    int* n_clients)
__attribute__ ((weak, alias ("_jet_ippc_partition_arinc_get_portal_type_info_impl")));

jet_ret_t _jet_ippc_partition_arinc_get_portal_info_impl(int server_portal_id,
    int* n_connections)
{
...
jet_ret_t _jet_ippc_partition_arinc_create_connections_impl(int server_portal_id,
    void* entry,
    size_t stack_size,
    int n_connections,
    pok_thread_id_t* thread_id)
...
jet_ret_t _jet_ippc_partition_arinc_return_impl(void)
{
...
jet_ret_t _jet_ippc_partition_arinc_copy_to_client_impl(void* dst,
    const void* src,
    size_t n)
{
...
jet_ret_t jet_ippc_partition_arinc_copy_to_client(void* dst,
    const void* src,
    size_t n)
__attribute__ ((weak, alias ("_jet_ippc_partition_arinc_copy_to_client_impl")));

jet_ret_t _jet_ippc_partition_arinc_copy_from_client_impl(void* dst,
    const void* src,
    size_t n)
{
...
jet_ret_t jet_ippc_partition_arinc_copy_from_client(void* dst,
    const void* src,
    size_t n)
