...
{
    registers[zero] = ea->r0;    /* zero */     /* Всегда содержит нуль, запись в регистр игнорируется */
    registers[AT] = ea->r1;      /*  at  */     /* Временные данные ассемблера (Assembler Temporaries) */
    registers[v0] = ea->r2;      /*  v0  */     /* Регистр результата (Value Holder) */
    registers[v1] = ea->r3;      /*  v1  */     /* Регистр результата (Value Holder) */
    registers[a0] = ea->r4;      /*  a0  */     /* Регистр для передаваемого аргумента (Argument) */
    registers[a1] = ea->r5;      /*  a1  */     /* Регистр для передаваемого аргумента (Argument) */
    registers[a2] = ea->r6;      /*  a2  */     /* Регистр для передаваемого аргумента (Argument) */
    registers[a3] = ea->r7;      /*  a3  */     /* Регистр для передаваемого аргумента (Argument) */
    registers[t0] = ea->r8;      /*  t0  */     /* Временные данные подпрограммы (Temporary) */
    registers[t1] = ea->r9;      /*  t1  */     /* Временные данные подпрограммы (Temporary) */
    registers[t2] = ea->r10;     /*  t2  */     /* Временные данные подпрограммы (Temporary) */
    registers[t3] = ea->r11;     /*  t3  */     /* Временные данные подпрограммы (Temporary) */
    registers[t4] = ea->r12;     /*  t4  */     /* Временные данные подпрограммы (Temporary) */
    registers[t5] = ea->r13;     /*  t5  */     /* Временные данные подпрограммы (Temporary) */
    registers[t6] = ea->r14;     /*  t6  */     /* Временные данные подпрограммы (Temporary) */
    registers[t7] = ea->r15;     /*  t7  */     /* Временные данные подпрограммы (Temporary) */
    registers[s0] = ea->r16;     /*  s0  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[s1] = ea->r17;     /*  s1  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[s2] = ea->r18;     /*  s2  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[s3] = ea->r19;     /*  s3  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[s4] = ea->r20;     /*  s4  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[s5] = ea->r21;     /*  s5  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[s6] = ea->r22;     /*  s6  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[s7] = ea->r23;     /*  s7  */     /* Регистр сохраняемый подпрограммой (Saved Register) */
    registers[t8] = ea->r24;     /*  t8  */     /* Временные данные подпрограммы (Temporary) */
    registers[t9] = ea->r25;     /*  t9  */     /* Временные данные подпрограммы (Temporary) */
    registers[jp] = ea->r26;     /*  k0  */     /* Регистр ядра ОС (Kernel Register) */
    registers[k0] = ea->r27;     /*  k1  */     /* Регистр ядра ОС (Kernel Register) */
    registers[k1] = ea->r28;     /*  gp  */     /* Указатель на секцию глобальных переменных (Global Pointer) */
    registers[gp] = ea->r29;     /*  sp  */     /* Указатель на вершину стека (Stack Pointer) */
    registers[sp] = ea->r30;     /* s8,fp*/    /* Указатель кадра (Frame Pointer) */
    registers[fp] = ea->r31;     /*  ra  */     /* Регистр адреса возврата из подпрограммы (Return Address) */
